...
function k(aq) {
    if ("Development" == dw) {
        if ("OneLine" == dj) log.innerHTML = aq;
        else if ("Cascade" == dj) log.innerHTML += "<br/>" + aq;
    }
};
...
function J() {
    if (bP >= am) {
        if (0 < kPX) kPX = 0;
        else if (kPX + C < t.width) kPX = t.width - C;
        if (V < t.height) {
            kPW = (t.height - V) >> 1;
        } else {
            if (0 < kPW) kPW = 0;
            else if (kPW + V < t.height) kPW = t.height - V;
        }
    } else {
        if (0 < kPW) kPW = 0;
        else if (kPW + V < t.height) kPW = t.height - V;
...
function l() {
    ...
    if (1 >= au && aB == bz && aJ == aH) {} else if (bA && bA[aH][bz].complete) {
        v(bz, aH);
        aB = bz;
        aJ = aH;
    }
...
function MouseDown(event) {
    ...
    clearTimeout(aN);
    G.bb((L.width - G.h.width) >> 1, L.height - G.h.width * 1.2, G.h.width, G.h.height, 10);
    var _bB = L.getBoundingClientRect();
    bs = event.clientX - _bB.left;
    bN = event.clientY - _bB.top;
    bJ = bs;
    as = bN;
    aY = bI.bV(bs, bN);
    if (!aY) {
...
function MouseMove(event) {
    ...
    aZ = C;
    ab = V;
    aC = event.clientX - _bB.left;
    aP = event.clientY - _bB.top;
    aX = aC - kPX;
    aT = aP - kPW;
    if (!aI) return;
    event = event || window.event;
    event.preventDefault();
    if (aV) {
        var x = event.clientX - _bB.left;
        var y = event.clientY - _bB.top;
        bJ = x;
        as = y;
        if (1 == aw) {
            var bp = Math.sqrt(Math.pow(x - ao, 2), Math.pow(y - bH, 2));
...
function F() {
    by = du.bd(L.width >> 2, 20, L.width >> 1, 60);
    bI.dz(by);
};
...
function H() {
    ...
    var bD = aD / 8;
    var aj = bD;
    G = OKQ.bd((L.width - bD) >> 1, L.height - aj * 1.2, bD, aj, "H5Player/UI/AutoSpinNormal.png", "H5Player/UI/AutoSpinPressed.png");
    G.OnClick = function() {
        if (0 == bB) {
            if (0 <= bk) bB = 7;
            else bB = -7;
    ...
    bd: function(x, y, width, height) {
        ...
        ak.aG = function() {
            ...
            if (100 <= ak.az) {
                aq = "已加载100%！";
                setTimeout(function() {
                    bQ.clearRect(0, ak.h.y - 50, L.width, ak.h.height << 1);
                    ak.visible = false;
                }, 1200);
            } else {
                aq = "已加载" + ak.az + "%，请稍候…";
            ...
            bQ.clearRect(0, ak.h.y - 50, L.width, ak.h.height << 1);
            bQ.font = "bold 40px 微软雅黑";
            bQ.textAlign = "center";
            bQ.textBaseline = "hanging";
            bQ.lineWidth = 2;
            bQ.strokeStyle = '#000000';
            var m = bQ.measureText(aq);
            bQ.strokeText(aq, ak.h.x + (ak.h.width >> 1), ak.h.y);
            bQ.fillStyle = '#0000FF';
            bQ.fillText(aq, ak.h.x + (ak.h.width >> 1), ak.h.y);
        };
        return ak;
    }
};
